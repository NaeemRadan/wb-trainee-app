  ...
  <div class="orders-view">
    ...
    <div v-if="filteredData" class="content-grid">
      ...
      <div class="chart-container">
        ...
        <BarChart v-if="ordersChartData" :chart-data="ordersChartData" />
      </div>
        ...
        <table>
          ...
          <tbody>
            <tr v-for="order in paginatedData" :key="order.orderId">
              <td>{{ order.orderId }}</td>
              <td>{{ new Date(order.creationDate).toLocaleString('ru-RU') }}</td>
              <td>{{ order.status }}</td>
              <td>{{ order.totalSum.toLocaleString('ru-RU') }} ₽</td>
...
const { data, loading, error, fetchData } = useApi('orders');
...
const ordersChartData = computed(() => {
  ...
  const ordersByStatus = filteredData.value.reduce((acc, order) => {
    if (!acc[order.status]) {
      acc[order.status] = 0;
    }
    acc[order.status]++;
...
.orders-view { padding: 20px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; max-width: 1200px; margin: 0 auto; }
