<template>
  <div class="incomes-view">
    <h1>Страница "Поступления" (Incomes)</h1>
    ...
    <div v-if="filteredData" class="content-grid">
      ...
      <div class="chart-container">
        <h2>Сумма поступлений</h2>
        <BarChart v-if="incomesChartData" :chart-data="incomesChartData" />
      </div>


      <div class="table-container">
        <div class="table-header">
          <h2>Детали поступлений</h2>
        ...
        <table>
          ...
          <tbody>
            <tr v-for="income in paginatedData" :key="income.incomeId">
              <td>{{ income.incomeId }}</td>
              <td>{{ new Date(income.date).toLocaleString('ru-RU') }}</td>
              <td>{{ income.documentNumber }}</td>
              <td>{{ income.quantity }}</td>
              <td>{{ income.totalSum.toLocaleString('ru-RU') }} ₽</td>
            </tr>
          </tbody>
        </table>
...
const { data, loading, error, fetchData } = useApi('incomes');
...
const filteredData = computed(() => {
  ...
  return data.value.filter(income => 
    income.documentNumber.toLowerCase().includes(lowerCaseSearch)
  );
});
...
const incomesChartData = computed(() => {
  if (!filteredData.value || filteredData.value.length === 0) return null;
  const labels = filteredData.value.map(income => income.documentNumber);
  const values = filteredData.value.map(income => income.totalSum);
  return {
    labels: labels,
    datasets: [{
...
.incomes-view { padding: 20px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; max-width: 1200px; margin: 0 auto; }
